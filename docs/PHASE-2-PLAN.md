...
| 7 | `feat(phase-2): student FSRS review session via ts-fsrs` (**superseded by Plan B** — see Phase 3 rewrite in `docs/PLAN.md`) | Initial Phase 2 ship of `/student/review` used a self-rating UI (Again / Hard / Good / Easy buttons). After this ship, the architect approved a pivot: review is driven by MCQ correctness, not self-rating. The new model collapses the daily quiz into the review flow — student answers MCQs from FSRS-due cards; correct → Good rating + 5 XP, wrong → Again rating. The self-rating UI is being rebuilt in Phase 3 commit #2; the `src/lib/fsrs.ts` adapter is reusable. See migration 015 + `docs/PLAN.md` §8 Phase 3. | (Was: student rates a card; `card_reviews.due_at` advances per FSRS.) The shipped self-rating UI was validated by the user, then deprecated by the Plan B pivot before Phase 3 began. |
