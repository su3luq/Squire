...
| 031 | Schedule `send-pushes` Edge Function via `pg_cron` (every minute). Reads URL + shared secret from `app.settings.functions_url` / `app.settings.cron_secret`. NOT applied automatically — see `docs/PUSH_SETUP.md` for the four-step activation (CRON_SECRET, Postgres settings, function secrets, deploy + migrate). | not applied |
